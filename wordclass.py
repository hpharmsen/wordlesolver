...
class Words:
    ...
    def match(self, pattern):
        return [
            ...
            if all([p in (w, "?") for w, p in zip(word, pattern)])
        ]

    def sort_on_letter_frequency(self):
        def tot_freq(word):
            return sum([self.frequencies[letter] for letter in list(set(word))])

        self.words.sort(key=tot_freq, reverse=True)
        return self

    def extended_match(
        self, pattern: str, unused_letters: set, wrongly_placed_letters: list[str]
    ):
        ...
        for word in self.words:
            ...
            if all(
                [
                    p in (w, "?") and w not in unused_letters
